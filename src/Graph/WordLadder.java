...
public class WordLadder {
	 ...
	 *  (avoid the circle in the graph) hit -> hot -> hit !!!!!!!!!!!
	 *  
			...
			while(currentLevelSize > 0){
				...
				for(String neighbour : neighbours){
					...
					if(neighbour.equals(end)){
						...
						//only put unvisited word and contained in dict to queue 
	...
	public int ladderLength2(String start, String end, HashSet<String> dict) {
        ...
        while(!q.isEmpty()){
        	...
        	while(currentLevelSize > 0){
                String word = q.poll();
                currentLevelSize--;
                ArrayList<String> neighbours = getNeighbours2(word);
                for(String neighbour: neighbours){
                	if(neighbour.equals(end)) return depth;
                    if(dict.contains(neighbour) && !visitedSet.contains(neighbour)){
                        dict.add(neighbour);
                        q.offer(neighbour);
                    }
                }
	...
	//¹ý²»ÁËlarge case
    public ArrayList<String> getNeighbours2(String start){
        ArrayList<String> neighbours = new ArrayList<String>();
        for(int i = 0; i < start.length(); i++){
            for(char c = 'a'; c <= 'z'; c++){
                if(start.charAt(i) != c){
                    String neighbour = start.substring(0, i) + c + start.substring(i+1, start.length());
	...
	public static void main(String[] args) {
		...
		System.out.println(wl.ladderLength2("hot", "dog", dict));
	}
